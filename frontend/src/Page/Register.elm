...
update msg model =
  case msg of
    ...
    RegisterCompleted (Err error) ->
      let
          errorMessages =
            [ "Something went wrong!" ] -- TODO implement proper error handling from error message
      in
...
viewForm model =
  form [ class "form-group row", onSubmit SubmitForm ]
       ...
       , viewTextField { id = "firstName", label = "First Name", value = model.firstName, msg = SetFirstName }
       , viewEmailField { id = "email", label = "Email", value = model.email, msg = SetEmail }
       , viewPasswordField { id = "password", label = "Password", value = model.password, msg = SetPassword }
       , div [ class "field" ]
             [ div [ class "control" ]
                   [ button [ class "button is-success is-fullwidth is-large is-outlined" ] [ text "Register" ] ]
             ]
